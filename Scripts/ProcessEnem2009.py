...
if Path("../Data/Processed/ENEM_2009_Concluintes.csv").exists():
    enem_df = pd.read_csv("../Data/Processed/ENEM_2009_Concluintes.csv")
    ...
    df.to_csv("../Data/Processed/ENEM_2009_Concluintes.csv", index = False)
    ...
    df.drop(['TP_ST_CONCLUSAO'], axis=1, inplace=True)
    df.to_csv("../Data/Processed/ENEM_2009_NaoConcluintes.csv", index = False)
    print(f"[INFO] {df.shape[0]} participantes não concluintes no total.")
    print('---------------------------')


questoes_pd = pd.read_csv("../Data/Original/microdados_enem2009/Dados/ITENS_PROVA_2009.csv",
                        sep=';')
...
for comp in competencias:
    ...
    df_comp.to_csv("../Data/Processed/ENEM_2009_Concluintes_"+comp+".csv", index = False)
