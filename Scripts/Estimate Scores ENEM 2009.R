...
for (comp in competencias) {
    ...
    ch <- read.csv(file=sprintf('../Data/Processed/ENEM2009/Concluintes_%s.csv',comp))
    ch.itens <- ch[,2:46]
    ch.true_scores <- ch[,1]
    ...
    file_name = sprintf("../Data/Processed/ENEM2009/Parameters/Parametros_%s_Concluintes.csv", comp)
    ...
    file_name = sprintf("../Data/Processed/ENEM2009/Estimated Scores/Concluintes_%s.csv", comp)
    if(file.exists(file_name)){
      ...
      write.csv(ch.scoret, sprintf("../Data/Processed/ENEM2009/Estimated Scores/Concluintes_%s_0_1000.csv", comp))
    ...
    png(file=sprintf("../Images/%s2009-Concluintes.png",comp))
